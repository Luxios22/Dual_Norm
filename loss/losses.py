...
class ConfidencePenalty(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        Args:
            ...
            targets: ground truth labels with shape (num_classes)
        """
        #log_probs = torch.log(inputs.clamp(min=1e-8,max=1.0))
        entropy=-torch.sum((inputs*torch.log(inputs.clamp(min=1e-8,max=1.0))),dim=1)
        #loss = (- Variable(targets) * log_probs).mean(0).sum()
        #entropy =-(inputs*log_probs).sum(dim=1)
