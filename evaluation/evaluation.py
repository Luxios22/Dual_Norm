...
def evaluation(all_feats,all_pids,all_camids,num_query,rr=False, max_rank=50, mode='euclidean'):
    ...
    if not rr:
        ...
        print('Calculating Distance - {}'.format(mode))
        if mode == 'dot':
            ...
            q_g_dist = cdist(qf.data.cpu(), gf.data.cpu())
            q_q_dist = cdist(qf.data.cpu(), qf.data.cpu())
            g_g_dist = cdist(gf.data.cpu(), gf.data.cpu())
        else:
            raise ValueError('Invalid evaluation mode.')
        print('Re-ranking:')
