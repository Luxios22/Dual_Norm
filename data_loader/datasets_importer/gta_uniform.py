...
class GTA_Uniform(BaseImageDataset):
    ...
    def __init__(self, cfg, verbose = True, relabel = False, **kwargs):
        ...
        self.query_list = os.path.join(self.dataset_dir, f'query_1.txt')
        self.gallery_list = os.path.join(self.dataset_dir, f'gallery_1.txt')
        print(f'The dataset directory is: {self.dataset_dir}')
        
        pid_files = ['train.txt',f'gallery_1.txt']
    ...
    def generate_label(self,fil):
        ...
        if fil == f'gallery_1.txt':
            q_file =  open(os.path.join(save_dir,f'query_1.txt'),'w')
