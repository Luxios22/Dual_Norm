...
class GTA_NonUniform_Unique(BaseImageDataset):
    """
    GTA Long-Term
    ...
    def __init__(self, cfg, verbose = True, relabel = False, **kwargs):
        ...
        super(GTA_NonUniform_Unique, self).__init__()
        ...
        self.query_list = os.path.join(self.dataset_dir, f'query_1_unique.txt')
        self.gallery_list = os.path.join(self.dataset_dir, f'gallery_1_unique.txt')
        print(f'The dataset directory is: {self.dataset_dir}')
        
        pid_files = ['train.txt',f'gallery_1_unique.txt']
    ...
    def generate_label(self,fil):
        ...
        if fil == f'gallery_1_unique.txt':
            q_file =  open(os.path.join(save_dir,f'query_1_unique.txt'),'w')
