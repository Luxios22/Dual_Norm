...
from .samplers import RandomIdentitySampler, SoftmaxBalancedSampler
...
def data_loader(cfg,dataset_name):
    ...
    if cfg.SOLVER.LOSS == 'softmax':
        if cfg.SOLVER.CAM_RESAMPLING == 'balanced':
            print('Using naive balanced sampling...')
            sampler = SoftmaxBalancedSampler(dataset.train)
            train_loader = DataLoader(train_set, batch_size=cfg.SOLVER.IMS_PER_BATCH,sampler=sampler, num_workers=num_workers, collate_fn=train_collate_fn)
        else:
